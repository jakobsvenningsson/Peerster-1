...
import (
	...
	"github.com/sagap/Peerster/hw3/part2/routing"
)
...
type Gossiper struct {
	...
	msgChnFS3   chan *messaging.SearchRequest  // channel accepting search requests
	msgChnFS4   chan *messaging.SearchReply  // channel accepting search requests
...
func NewGossiper(address, name string, peers string, rtimer, guiPort int, noforward bool) (*Gossiper) {
	...
	gossiper := &Gossiper{
		...
		msgChnFS3:	 make(chan *messaging.SearchRequest),
		msgChnFS4:	 make(chan *messaging.SearchReply),
		setpeers:    peersList,
	...
	gossiper.FS = *gossiper.NewFileSharing(gossiper.msgChnFS, gossiper.msgChnFS2, gossiper.msgChnFS3, gossiper.msgChnFS4) // FileSharing Mechanism
...
func (gossiper *Gossiper) handleRequests(recv []byte, n int, udpAddr string, flag bool, addr *net.UDPAddr) {
	...
	if gossiper.checkIfClient(t2){
		...
		if t2.Status != nil {
			...
			if t2.Reply.Destination == gossiper.origin {
				fmt.Println("REPLY")
				strKey := t2.Reply.Origin + "," + t2.Reply.FileName
				gossiper.FS.channelsToReceive[strKey] <- t2.Reply
			} else {
				fmt.Println("FORWARD REPLY")
				t2.Reply.HopLimit--
				if t2.Reply.HopLimit == 0 {
					return
				} else {
					messageToSend := messaging.GossipPacket{nil, nil, nil, nil, t2.Reply,nil,nil,nil}
			...
			if strings.Compare(t2.Request.Destination,"") == 0 && strings.Compare(string(t2.Request.HashValue),"") != 0{
				...
				if empty == 0 {
					...
					fmt.Println("REQUEST")
					gossiper.msgChnFS2 <- t2.Request                 // request from another peer
				}
			} else {
				...
				if t2.Request.HopLimit == 0 {
					...
					messageToSend := messaging.GossipPacket{nil, nil, nil, t2.Request, nil,nil,nil,nil}
					go gossiper.sendPrivateMessages(messageToSend, t2.Request.Destination)
				}
			}
		}else if t2.SearchReq != nil{
			if strings.Compare(t2.SearchReq.Origin,"") == 0{				// search req just created
				t2.SearchReq.Origin = gossiper.origin
				go gossiper.performSearch(*t2.SearchReq)
			}else{			// search request comes from peerster node
				if gossiper.checkDuplicates(*t2.SearchReq){
					gossiper.msgChnFS3 <- t2.SearchReq
				}
			}
			...
			if strings.Compare(t2.SearchRep.Destination, gossiper.origin) == 0{
				...
				t2.SearchRep.HopLimit--
				if t2.SearchRep.HopLimit == 0 {
					return
				} else {
					messageToSend := messaging.GossipPacket{nil, nil, nil, nil, nil,nil,t2.SearchRep,nil}
					go gossiper.sendPrivateMessages(messageToSend, t2.SearchRep.Destination)
				}
			}
		}
	}
		...
		for msg := range gossiper.msgChn2 {
			if gossiper.checkMessageType(msg.Msg) { // RumorMessage
				...
				if !gossiper.noForward || strings.Compare(msg.Msg.Rumor.Text, "") == 0 {
					toSend := messaging.GossipPacket{nil, &messaging.StatusPacket{gossiper.CreateStatusPacket2()}, nil, nil, nil,nil,nil,nil}
...
func (gossiper *Gossiper) sendRumors(msg messaging.RumorMessage, peers []string) {
	var message = &messaging.GossipPacket{&msg, nil, nil, nil, nil,nil,nil,nil}
...
func (gossiper *Gossiper) announceMyself() {
	...
	gossiper.storeVectorClocks(messaging.GossipPacket{&rumor, nil, nil, nil, nil,nil,nil,nil})
