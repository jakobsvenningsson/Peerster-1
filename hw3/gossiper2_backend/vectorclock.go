...
func (gossiper *Gossiper) CompareVectorClocks(msg messaging.MsgReceiver) bool{
	...
	for index := range msg.Msg.Status.Want {
		...
		if ok{
			flag = false
			tempPort := 0
			newPacket := messaging.GossipPacket{&messaging.RumorMessage{tempOrigin,msg.Msg.Status.Want[index].NextID,tempVC, &net.IP{}, &tempPort}, nil, nil,nil,nil}
			...
			for true{
				...
				if ok1{
					newPacket := messaging.GossipPacket{&messaging.RumorMessage{tempOrigin,counter,tempText, &net.IP{}, &tempPort}, nil, nil,nil,nil}
					gossiper.sendMessages(newPacket, msg.UpdAddr)
				}else{
					break
				}
			...
			if once{
				...
				if !ok {
					...
					newPacket := messaging.GossipPacket{nil, &messaging.StatusPacket{gossiper.CreateStatusPacket2()}, nil,nil,nil}
					gossiper.sendMessages(newPacket, msg.UpdAddr)
	...
	for key, value := range gossiper.Vc.store{
		...
		if !ok{
			...
			for true{
				...
				if ok{
					newPacket := messaging.GossipPacket{&messaging.RumorMessage{key,uint32(counter),tempText, &net.IP{}, &tempPort}, nil, nil,nil,nil}
					gossiper.sendMessages(newPacket, msg.UpdAddr)
					counter++
				}else{
					break
